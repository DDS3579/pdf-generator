...
class PageFurnitureRenderer {
    ...
    render() {
        ...
        const coverPages = Number.isFinite(coverPagesRaw) && coverPagesRaw >= 0
            ? Math.floor(coverPagesRaw)
            : 0;

        const numberedTotalPages = Math.max(
            0,
            physicalTotalPages - coverPages
        );
        ...
        for (
            ...
            const displayPageNumber =
                physicalPageNumber - coverPages + start - 1;
            ...
            const context = {
                ...
                page: displayPageNumber > 0
                    ...
                    : "",
                totalPages: numberedTotalPages > 0
                    ? String(numberedTotalPages)
                    : ""
            };
    ...
    _renderSection(sectionName, payload) {
        ...
        const y = sectionName === "header"
            ? layout.margins.top * 0.45
            : layout.pageHeight - layout.margins.bottom * 0.55;

        pdf.font(style.fontFamily).fontSize(style.fontSize);

        const leftText = this._prepareText(
            config.left,
            payload.context,
            style
        );

        const centerText = this._prepareText(
            config.center,
            payload.context,
            style
        );

        const rightText = this._prepareText(
            config.right,
            payload.context,
            style
        );
        ...
        if (config.rule) {
            const ruleY = sectionName === "header"
    ...
    _getConfig(sectionName, isCoverPage, physicalPageNumber) {
        ...
        if (isCoverPage && base.coverPage) {
            override = base.coverPage;
