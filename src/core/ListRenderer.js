...
class ListRenderer {
    ...
    render(items, options = {}) {
        ...
        if (ordered && (!Number.isInteger(start) || start < 1)) {
            throw new Error("options.start must be a positive integer for numbered lists.");
        }
        ...
        // Add spacing before the list, unless we are already at the top of a page.
        if (
        ...
        // Set list font before measuring markers.
        pdf.font(style.fontFamily).fontSize(style.fontSize);

        // For ordered lists, reserve enough horizontal space for the largest number.
        // Example: if the list ends at "12.", reserve width for "12.".
    ...
    _renderNode(node, style, orderedMarkerWidth) {
        ...
        if (node.ordered) {
            ...
            // Right-align the number inside the reserved marker column.
